...
class TestToolIntegration:
    ...
    def test_insert_run_with_last_insert(self, session_id):
        ...
        result = docx_insert_run(session_id, "Bold text", position="inside:last_insert")
        assert is_success(result)
    ...
    def test_update_paragraph_with_last_insert(self, session_id):
        ...
        result = docx_update_paragraph_text(session_id, "last_insert", "Updated text")
        assert is_success(result)
    ...
    def test_set_alignment_with_last_update(self, session_id):
        ...
        result = docx_update_paragraph_text(session_id, para_id, "Updated")
        assert is_success(result)

        # Set alignment using last_update
        result = docx_set_alignment(session_id, "last_update", "center")
        assert is_success(result)
    ...
    def test_format_copy_with_special_ids(self, session_id):
        ...
        result = docx_insert_paragraph(session_id, "Source", position="end:document_body")
        source_id = extract_element_id(result)

        # Create and update target paragraph
        result = docx_insert_paragraph(session_id, "Target", position="end:document_body")
        target_id = extract_element_id(result)

        result = docx_update_paragraph_text(session_id, target_id, "Updated target")
        assert is_success(result)
        ...
        result = docx_format_copy(session_id, source_id, "last_update")
        assert is_success(result)
    ...
    def test_special_id_not_available_error(self, session_id):
        ...
        result = docx_update_paragraph_text(session_id, "last_insert", "Text")
    ...
    def test_case_insensitive_special_ids(self, session_id):
        ...
        result = docx_update_paragraph_text(session_id, "LAST_INSERT", "Updated")
        assert is_success(result)

        # Now last_update should be set, use mixed case to update again
        result = docx_update_paragraph_text(session_id, "Last_Update", "Updated again")
        assert is_success(result)

        # Verify we can still use LAST_UPDATE
        result = docx_update_paragraph_text(session_id, "LAST_UPDATE", "Final update")
        assert is_success(result)
...
class TestMultipleSpecialIDs:
    ...
    def test_last_insert_and_last_update_tracking(self, session_id, session):
        ...
        result = docx_insert_paragraph(session_id, "Para 1", position="end:document_body")
        para1_id = extract_element_id(result)

        # Create second paragraph
        result = docx_insert_paragraph(session_id, "Para 2", position="end:document_body")
        para2_id = extract_element_id(result)

        # Update first paragraph
        result = docx_update_paragraph_text(session_id, para1_id, "Para 1 updated")
        assert is_success(result)
    ...
    def test_cursor_with_last_insert(self, session_id, session):
        ...
        result = docx_insert_paragraph(session_id, "Para 2", position="after:cursor")
        assert is_success(result)

        # Verify both cursor and last_insert work
        result = docx_insert_paragraph(session_id, "Para 3", position="after:last_insert")
        assert is_success(result)
