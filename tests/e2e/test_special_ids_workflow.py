...
def test_consecutive_insertion_with_last_insert():
    ...
    p1_resp = docx_insert_paragraph(session_id, "First paragraph", position="end:document_body")
    ...
    p2_resp = docx_insert_paragraph(session_id, "Second paragraph", position="after:last_insert")
    assert is_success(p2_resp)
    p2_id = extract_element_id(p2_resp)

    # Insert third paragraph using last_insert again
    p3_resp = docx_insert_paragraph(session_id, "Third paragraph", position="after:last_insert")
    assert is_success(p3_resp)
    p3_id = extract_element_id(p3_resp)

    # Insert table using last_insert
    table_resp = docx_insert_table(session_id, 2, 2, position="after:last_insert")
...
def test_cursor_positioning():
    ...
    p2_resp = docx_insert_paragraph(session_id, "Paragraph 2", position="after:last_insert")
    assert is_success(p2_resp)

    p3_resp = docx_insert_paragraph(session_id, "Paragraph 3", position="after:last_insert")
    assert is_success(p3_resp)

    # Now insert using cursor (which should be at p3)
    p4_resp = docx_insert_paragraph(session_id, "Paragraph 4", position="after:cursor")
...
def test_format_copy_with_special_ids():
    ...
    run1_resp = docx_insert_run(session_id, "Bold Red Text", position=f"inside:{p1_id}")
    assert is_success(run1_resp)
    run1_id = extract_element_id(run1_resp)

    # Format the run (this updates last_update to run1_id)
    font_resp = docx_set_font(session_id, run1_id, bold=True, color_hex="FF0000", size=16)
    ...
    run2_resp = docx_insert_run(session_id, "Normal Text", position=f"inside:{p2_id}")
    ...
    format_resp = docx_format_copy(session_id, source_id=run1_id, target_id="last_insert")
...
def test_error_recovery_uninitialized_special_ids():
    ...
    p1_resp = docx_insert_paragraph(session_id, "Test", position="after:last_insert")
    ...
    p2_resp = docx_insert_paragraph(session_id, "Test", position="after:last_update")
    ...
    p3_resp = docx_insert_paragraph(session_id, "Valid", position="end:document_body")
    assert is_success(p3_resp)

    # Now last_insert should work
    p4_resp = docx_insert_paragraph(session_id, "After valid", position="after:last_insert")
    assert is_success(p4_resp)
...
def test_special_ids_with_file_save():
    ...
    p1_resp = docx_insert_paragraph(session_id, "Title", position="end:document_body")
    assert is_success(p1_resp)

    p2_resp = docx_insert_paragraph(session_id, "Content 1", position="after:last_insert")
    assert is_success(p2_resp)

    p3_resp = docx_insert_paragraph(session_id, "Content 2", position="after:last_insert")
    assert is_success(p3_resp)
    ...
    try:
        save_resp = docx_save(session_id, tmp_path)
...
def test_mixed_special_and_explicit_ids():
    ...
    p3_resp = docx_insert_paragraph(session_id, "Paragraph 1.5", position=f"after:{p1_id}")
    assert is_success(p3_resp)
    p3_id = extract_element_id(p3_resp)

    # Insert using last_insert (should be after p3)
    p4_resp = docx_insert_paragraph(session_id, "Paragraph 1.75", position="after:last_insert")
    assert is_success(p4_resp)
