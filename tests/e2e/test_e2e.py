...
class TestEndToEnd(unittest.TestCase):
    def test_full_document_creation(self):
        ...
        # 2. Add Title
        title_resp = docx_insert_heading("Monthly Report", position="end:document_body", level=0)
        title_id = _extract(title_resp)["element_id"]
        self.assertTrue(title_id.startswith("para_"))

        # 3. Add Intro Paragraph
        para_resp = docx_insert_paragraph("This is an automated report generated by ", position="end:document_body")
        para_id = _extract(para_resp)["element_id"]
        run_resp = docx_insert_run("Claude", position=f"inside:{para_id}")
        run_id = _extract(run_resp)["element_id"]

        # 4. Style the "Claude" run
        docx_set_font(run_id, bold=True, color_hex="0000FF")

        # 5. Add a Data Table
        docx_insert_heading("Data Summary", position="end:document_body", level=1)
        table_resp = docx_insert_table(rows=2, cols=2, position="end:document_body")
        table_id = _extract(table_resp)["element_id"]

        # Header Row
        c00_resp = docx_get_cell(table_id, 0, 0)
        c00 = _extract(c00_resp)["element_id"]
        docx_insert_paragraph_to_cell("Metric", position=f"inside:{c00}")

        c01_resp = docx_get_cell(table_id, 0, 1)
        c01 = _extract(c01_resp)["element_id"]
        docx_insert_paragraph_to_cell("Value", position=f"inside:{c01}")

        # Data Row
        c10_resp = docx_get_cell(table_id, 1, 0)
        c10 = _extract(c10_resp)["element_id"]
        docx_insert_paragraph_to_cell("Revenue", position=f"inside:{c10}")

        c11_resp = docx_get_cell(table_id, 1, 1)
        c11 = _extract(c11_resp)["element_id"]
        docx_insert_paragraph_to_cell("$1,000,000", position=f"inside:{c11}")
