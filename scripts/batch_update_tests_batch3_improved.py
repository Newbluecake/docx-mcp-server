#!/usr/bin/env python3
"""Improved batch update script with AST-based import detection and validation."""
...
def find_import_end_position(content: str) -> int:
    """Find the end position of import section using AST."""
    try:
        tree = ast.parse(content)
    except SyntaxError:
        # Fallback to line-based detection if AST fails
        return find_import_end_fallback(content)
    ...
    lines = content.split('\n')
    return sum(len(line) + 1 for line in lines[:last_import_lineno])


def find_import_end_fallback(content: str) -> int:
    """Fallback method using line-based detection with multi-line support."""
    lines = content.split('\n')
    import_end = 0
    ...
    for i, line in enumerate(lines):
        ...
        elif import_end > 0 and stripped == '':
            continue
    ...
    # Convert line number to character position
    return sum(len(line) + 1 for line in lines[:import_end])
...
def update_imports(content: str, file_path: Path) -> Tuple[str, bool]:
    ...
    # Find import section end using AST
    lines = content.split('\n')
    import_end_line = 0
    in_multiline_import = False
...
def main():
    ...
    # Files that were reverted in Batch 3
    files = [
    ...
    print("Batch updating reverted files with improved script...")
    print()
