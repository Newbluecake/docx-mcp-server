...
| R-004 | 基于 Cursor 的写入 | P0 | 作为开发者，当我调用 `add_paragraph` 等写入接口时，内容应插入在当前激活的 Cursor 位置，且 Cursor 自动后移。 |
...
- **AND** 连续调用 `docx_add_paragraph` 三次
- **THEN** 文档中应按顺序出现这三个段落
...
| F-004 | 修改 `docx_add_paragraph` | 重构现有接口，使其默认使用 Active Cursor 位置插入 | P0 | R-004 | ☐ |
| F-005 | 修改 `docx_add_table` | 重构现有接口，使其默认使用 Active Cursor 位置插入 | P0 | R-004 | ☐ |
| F-006 | 修改 `docx_add_run` | 重构现有接口，使其默认使用 Active Cursor 位置插入 | P0 | R-004 | ☐ |
